...
public class GameScreen implements Screen { // Implementa la interfaz de Screen, es decir, se comportara con las caracteristicas de una pantalla
	...
	private Texture texturaFondo, texturaPinguino, texturaPinguinoMuerto, texturaRoca, texturaInicio; // Una Texture es una clase que envuelve una textura estandar de OpenGL, se utiliza para imagenes simples.
	...
	private boolean inicial; //Booleano para mostrar la pantalla de inicio, antes del juego
	...
	public GameScreen(slipperyPenguin slipperypenguin) { // Constructor de la clase.
		...
		texturaInicio = new Texture("data/pantalla.png");
		texturaInicio.setFilter(TextureFilter.Linear, TextureFilter.Linear);
	...
	public void render(float delta){
		...
		if(inicial){
			...
			font.draw(batch, "Tocar para iniciar.", 50, 220);
	...
	private void renderJuegoNormal() {
		...
		if(puntuacion<12){
			font.draw(batch, "Easy Mode",(Gdx.graphics.getWidth()/2)-50,30);
		}else{
			if(puntuacion<33){
				font.draw(batch, "Normal Mode",(Gdx.graphics.getWidth()/2)-50,30);
			}else{
				if(puntuacion<42){
					font.draw(batch, "Hard Mode",(Gdx.graphics.getWidth()/2)-50,30);
				}else{
					if(puntuacion<66){
						font.draw(batch, "Insane Mode",(Gdx.graphics.getWidth()/2)-50,30);
					}else{
						if(puntuacion>=66){
							font.draw(batch, "Inmortal Mode",(Gdx.graphics.getWidth()/2)-50,30);
						}
					}
				}
			}
	...
	private void actualizarRocas() { // Método que sirve para ir actualizando los valores de las rocas. Se van reutilizando.
		if(roca1Activa) { // Si la roca1 esta activa
			roca1.update(puntuacion); // Actualizamos
